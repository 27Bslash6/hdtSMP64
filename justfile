...
build config=default_config:
    ...
    & "{{msbuild}}" hdtSMP64.sln -p:Configuration={{config}} -p:Platform=x64 -v:m -m

# Full rebuild (clean + build)
rebuild config=default_config:
    @echo "Rebuilding {{config}}..."
    & "{{msbuild}}" hdtSMP64.sln -p:Configuration={{config}} -p:Platform=x64 -v:m -m -t:Rebuild
...
profile config=default_config:
    ...
    & "{{msbuild}}" hdtSMP64.sln -p:Configuration={{config}} -p:Platform=x64 "-p:ForceImportBeforeCppTargets={{justfile_directory()}}\hdtSMP64\Tracy.props" -v:m -m
