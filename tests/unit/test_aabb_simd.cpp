...
// Highway tests disabled in standalone test build
// They require linking against the full hdtSMP64 library
// The Highway implementation is tested via the main build's integration tests
#define HIGHWAY_TESTS_ENABLED 0
...
namespace test
{
	...
	{
		return Aabb(_mm_set_ps(0, minZ, minY, minX), _mm_set_ps(0, maxZ, maxY, maxX));
	}
...
TEST_CASE("Aabb::collideWith2 AVX2 batch", "[aabb][avx2]")
{
	...
	{
		Aabb testCases[] = {
			makeAabb(5, 5, 5, 15, 15, 15),			makeAabb(-5, -5, -5, 5, 5, 5), makeAabb(20, 20, 20, 30, 30, 30),
			makeAabb(-30, -30, -30, -20, -20, -20), makeAabb(0, 0, 0, 10, 10, 10), makeAabb(9, 9, 9, 11, 11, 11),
		};
...
TEST_CASE("Aabb::collideWithMany batch processing", "[aabb][avx2]")
{
	...
	{
		Aabb aabbs[] = {makeAabb(5, 5, 5, 15, 15, 15), makeAabb(-5, -5, -5, 5, 5, 5)};
		std::vector<Aabb*> results;
...
TEST_CASE("Aabb::mergeMany AVX2 batch merge", "[aabb][avx2]")
{
	...
	SECTION("Merge two AABBs")
	{
		...
		Aabb toMerge[] = {makeAabb(-5, -5, -5, 5, 5, 5), makeAabb(8, 8, 8, 20, 20, 20)};
	...
	SECTION("Merge odd number of AABBs")
	{
		...
		Aabb toMerge[] = {makeAabb(0, 0, 0, 10, 10, 10), makeAabb(20, 20, 20, 30, 30, 30),
...
// ============================================================================
// Highway AABB batch collision tests
// ============================================================================
// These tests are disabled in the standalone test build because they require
// linking against the full hdtSMP64 library with Highway support.
// The Highway AABB implementation is validated through:
// 1. The build succeeding (compilation validation)
// 2. In-game testing with Tracy profiling
// 3. The algorithm matching the scalar version (same comparison logic)
#if HIGHWAY_TESTS_ENABLED
...
// Cast test::Aabb to hdt::Aabb - they have identical memory layout
// Both are struct { __m128 m_min; __m128 m_max; }
static const hdt::Aabb& toHdtAabb(const test::Aabb& a)
{
	return reinterpret_cast<const hdt::Aabb&>(a);
}

TEST_CASE("Highway::batchCollideWith basic functionality", "[aabb][highway]")
{
	using namespace test;
	...
	SECTION("Empty array returns 0")
	{
		...
		size_t count = hdt::highway::batchCollideWith(toHdtAabb(ref), nullptr, 0, resultBits);
		REQUIRE(count == 0);
		REQUIRE(resultBits[0] == 0);
	}

	SECTION("Single colliding AABB")
	{
		Aabb ref = makeAabb(0, 0, 0, 10, 10, 10);
		Aabb candidates[] = {makeAabb(5, 5, 5, 15, 15, 15)};
		uint64_t resultBits[1] = {0};

		size_t count = hdt::highway::batchCollideWith(toHdtAabb(ref), reinterpret_cast<const hdt::Aabb*>(candidates), 1,
													  resultBits);

		REQUIRE(count == 1);
		REQUIRE((resultBits[0] & 1) != 0);
	}

	SECTION("Single non-colliding AABB")
	{
		Aabb ref = makeAabb(0, 0, 0, 10, 10, 10);
		Aabb candidates[] = {makeAabb(20, 20, 20, 30, 30, 30)};
		uint64_t resultBits[1] = {0};
	...
	SECTION("Mixed collisions")
	{
		Aabb ref = makeAabb(0, 0, 0, 10, 10, 10);
		Aabb candidates[] = {
			makeAabb(5, 5, 5, 15, 15, 15),			// 0: collides
			makeAabb(20, 20, 20, 30, 30, 30),		// 1: doesn't
			makeAabb(-5, -5, -5, 5, 5, 5),			// 2: collides
			makeAabb(-30, -30, -30, -20, -20, -20), // 3: doesn't
			makeAabb(9, 9, 9, 11, 11, 11),			// 4: collides
		};
		uint64_t resultBits[1] = {0};

		size_t count = hdt::highway::batchCollideWith(toHdtAabb(ref), reinterpret_cast<const hdt::Aabb*>(candidates), 5,
													  resultBits);

		REQUIRE(count == 3);
		REQUIRE((resultBits[0] & (1ULL << 0)) != 0); // 0 collides
		REQUIRE((resultBits[0] & (1ULL << 1)) == 0); // 1 doesn't
		REQUIRE((resultBits[0] & (1ULL << 2)) != 0); // 2 collides
		REQUIRE((resultBits[0] & (1ULL << 3)) == 0); // 3 doesn't
		REQUIRE((resultBits[0] & (1ULL << 4)) != 0); // 4 collides
	}
}

TEST_CASE("Highway::batchCollideWith matches scalar version", "[aabb][highway]")
{
	using namespace test;

	SECTION("Small batch (under threshold)")
	{
		Aabb ref = makeAabb(0, 0, 0, 10, 10, 10);
		Aabb candidates[8];
		for (int i = 0; i < 8; i++) {
			float offset = static_cast<float>(i * 3 - 10);
			candidates[i] = makeAabb(offset, offset, offset, offset + 8, offset + 8, offset + 8);
		}

		uint64_t resultBits[1] = {0};
		size_t hwCount = hdt::highway::batchCollideWith(toHdtAabb(ref), reinterpret_cast<const hdt::Aabb*>(candidates),
														8, resultBits);

		// Compare with scalar
		size_t scalarCount = 0;
		for (int i = 0; i < 8; i++) {
			bool hwResult = (resultBits[0] & (1ULL << i)) != 0;
			bool scalarResult = ref.collideWith(candidates[i]);
			REQUIRE(hwResult == scalarResult);
			if (scalarResult)
				scalarCount++;
		}
		REQUIRE(hwCount == scalarCount);
	}

	SECTION("Medium batch (above threshold)")
	{
		Aabb ref = makeAabb(0, 0, 0, 10, 10, 10);
		Aabb candidates[32];
		for (int i = 0; i < 32; i++) {
			float offset = static_cast<float>(i * 2 - 30);
			candidates[i] = makeAabb(offset, offset, offset, offset + 8, offset + 8, offset + 8);
		}
	...
	SECTION("Large batch (64+ elements)")
	{
		Aabb ref = makeAabb(0, 0, 0, 10, 10, 10);
		Aabb candidates[100];
		for (int i = 0; i < 100; i++) {
			float offset = static_cast<float>(i * 1.5f - 70);
			candidates[i] = makeAabb(offset, offset, offset, offset + 8, offset + 8, offset + 8);
		}
		...
		size_t scalarCount = 0;
		for (int i = 0; i < 100; i++) {
			bool hwResult = (resultBits[i / 64] & (1ULL << (i % 64))) != 0;
			bool scalarResult = ref.collideWith(candidates[i]);
			REQUIRE(hwResult == scalarResult);
			if (scalarResult)
				scalarCount++;
...
TEST_CASE("Highway::batchCollideWith edge cases", "[aabb][highway]")
{
	using namespace test;

	SECTION("Touching AABBs (boundary case)")
	{
		// AABBs that touch but don't overlap
		Aabb ref = makeAabb(0, 0, 0, 10, 10, 10);
		Aabb candidates[] = {
			makeAabb(10, 0, 0, 20, 10, 10), // Touches on X
			makeAabb(0, 10, 0, 10, 20, 10), // Touches on Y
			makeAabb(0, 0, 10, 10, 10, 20), // Touches on Z
		};
		uint64_t resultBits[1] = {0};
		...
		// Touching AABBs should NOT collide (< is strict, not <=)
		// Using original collision logic: !(flag & 0x7) where flag = cmplt_ps
		for (int i = 0; i < 3; i++) {
			bool hwResult = (resultBits[0] & (1ULL << i)) != 0;
			bool scalarResult = ref.collideWith(candidates[i]);
			REQUIRE(hwResult == scalarResult);
	...
	{
		Aabb ref = makeAabb(5, 5, 5, 15, 15, 15);
		Aabb candidates[] = {makeAabb(5, 5, 5, 15, 15, 15)};
		uint64_t resultBits[1] = {0};
	...
	SECTION("Contained AABBs")
	{
		Aabb ref = makeAabb(0, 0, 0, 100, 100, 100);
		Aabb candidates[] = {
			makeAabb(25, 25, 25, 75, 75, 75), // Contained
			makeAabb(50, 50, 50, 60, 60, 60), // Also contained
		};
		uint64_t resultBits[1] = {0};

		size_t count = hdt::highway::batchCollideWith(toHdtAabb(ref), reinterpret_cast<const hdt::Aabb*>(candidates), 2,
													  resultBits);

		REQUIRE(count == 2);
		REQUIRE((resultBits[0] & 0b11) == 0b11);
	}

	SECTION("All AABBs collide")
	{
		Aabb ref = makeAabb(0, 0, 0, 100, 100, 100);
		Aabb candidates[20];
		for (int i = 0; i < 20; i++) {
			// All inside the reference AABB
			candidates[i] = makeAabb(static_cast<float>(i * 4), static_cast<float>(i * 4), static_cast<float>(i * 4),
									 static_cast<float>(i * 4 + 10), static_cast<float>(i * 4 + 10),
									 static_cast<float>(i * 4 + 10));
		}
		uint64_t resultBits[1] = {0};

		size_t count = hdt::highway::batchCollideWith(toHdtAabb(ref), reinterpret_cast<const hdt::Aabb*>(candidates),
													  20, resultBits);

		REQUIRE(count == 20);
		// All 20 bits should be set
		REQUIRE((resultBits[0] & 0xFFFFF) == 0xFFFFF);
	}

	SECTION("No AABBs collide")
	{
		Aabb ref = makeAabb(0, 0, 0, 10, 10, 10);
		Aabb candidates[20];
		for (int i = 0; i < 20; i++) {
			// All far away
			candidates[i] = makeAabb(100.0f + i * 20, 100.0f + i * 20, 100.0f + i * 20, 110.0f + i * 20,
									 110.0f + i * 20, 110.0f + i * 20);
		}
		uint64_t resultBits[1] = {0};
...
TEST_CASE("Highway::getSimdTargetName returns valid string", "[aabb][highway]")
{
