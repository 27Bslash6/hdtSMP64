...
namespace hdt
{
	...
	inline __m128 setAll(float f)
	{
		return pshufd<0>(_mm_load_ss(&f));
	}
	inline __m128 setAll0(__m128 m)
	{
		return pshufd<0>(m);
	}
	inline __m128 setAll1(__m128 m)
	{
		return pshufd<0x55>(m);
	}
	inline __m128 setAll2(__m128 m)
	{
		return pshufd<0xAA>(m);
	}
	inline __m128 setAll3(__m128 m)
	{
		return pshufd<0xFF>(m);
	}
	...
	inline int aligned(int x, int a)
	{
		return (x + a - 1) & -a;
	}
	...
	public:
		...
		btQsTransform(const btQuaternion& r, const btVector3& t, float s = 1.0f) : m_basis(r), m_originScale(t.get128())
		{
			setScale(s);
		}
		...
		btQsTransform(const btQsTransform& rhs) : m_basis(rhs.m_basis), m_originScale(rhs.m_originScale.get128())
		{
			assert(getScale() > 0);
		}

		btQuaternion getBasis() const
		{
			return m_basis;
		}
		btQuaternion& getBasis()
		{
			return m_basis;
		}
		void setBasis(const btQuaternion& q)
		{
			m_basis = q;
		}
		void setBasis(const btMatrix3x3& m)
		{
			m.getRotation(m_basis);
		}

		float getScale() const
		{
			return m_originScale[3];
		}
		float& getScale()
		{
			return m_originScale[3];
		}
		float getScaleReg() const
		{
			return _mm_cvtss_f32(setAll3(m_originScale.get128()));
		}
	...
	public:
		...
		{
			return ((btMatrix3x3*)this)->transpose();
		}
		btTransform toTransform() const
		{
			return btTransform(((btMatrix3x3*)this)->transpose(), m_col[3]);
		}
		/*inline btMatrix4x3& operator =(const btMatrix4x3& rhs)
		{
		_mm_store_ps(m_row[0].m128_f32, rhs.m_row[0]);
	...
	namespace ref
	{
		inline void retain(RefObject* o)
		{
			o->retain();
		}
		inline void release(RefObject* o)
		{
			o->release();
		}
	} // namespace ref
